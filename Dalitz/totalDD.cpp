...
void totalDD() {
	...
	//Create output file
	TFile* output = new TFile("/afs/cern.ch/work/y/yuxiao/public/Dalitz/dalitz_total_DD.root","RECREATE");
    	//TTree* summary_tree = tree->CloneTree(0);

	TH2F* plot = new TH2F("Dalitz",";m^{2}(D^{-}K_{s})[MeV^{2}/c^{4}];m^{2}(K_{s}#pi)[MeV^{2}/c^{4}]",100,5e+06,35e+06,100,0,18e+06);
