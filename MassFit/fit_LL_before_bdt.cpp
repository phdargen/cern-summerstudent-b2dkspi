...
{   
	...
	RooRealVar Ks_MM("Ks_MM","Ks_MM",0.);
	KsCat.defineType("LL",0);
	RooArgList list =  RooArgList(B_DTF_MM,BDTG,KsCat,Ks_MM);
	RooDataSet*  data = new RooDataSet("data","data",tree,list, " (KsCat == 0) && (BDTG > -0.8512) && (Ks_MM>468.) && (Ks_MM<525.)" );
	...
	pdf.plotOn(frame_m,Components(*cheby),LineColor(6),LineStyle(1), Name("bkg"));	
	...
	B_DTF_MM.setRange("signal",5229.65,5329.65) ;
