...
*/	
	...
	//Bs part.reco.bkg
	RooRealVar mean4("mean4", "mean4", 5207.3, 5207.3-5*2.48,5207.3+5*2.48);
	RooRealVar sigma4("sigma4", "sigma4", 18.963,18.963-5*1.43,18.963+5*1.43);
	RooGaussian Gauss4("Gauss4", "Gauss4", B_DTF_MM, mean4, sigma4);

	RooRealVar mean5("mean5", "mean5", 5164.2,5164.2-5*3.08,5164.2+5*3.08);
	RooRealVar sigma5("sigma5", "sigma5", 26.953,26.953-5*2.21,26.953+5*2.21);
	RooGaussian Gauss5("Gauss5", "Gauss5", B_DTF_MM, mean5, sigma5);

	RooRealVar alpha4("alpha4","alpha4",7.2323e-01,7.2323e-01-5*0.16,7.2323e-01+5*0.16);
	RooRealVar alpha5("alpha5","alpha5",-1.5078,-1.5078-5*0.157,-1.5078+5*0.157);

	RooRealVar n4("n4","n4",2.9811,2.9811-5*1.02,2.9811+5*1.02);
	RooRealVar n5("n5","n5",1.1430,1.1430-5*0.159,1.1430+5*0.159);

	RooCBShape CB4("CB4","CB4",B_DTF_MM,mean4,sigma4, alpha4, n4);
	RooCBShape CB5("CB5","CB5",B_DTF_MM,mean5,sigma5, alpha5, n5);

	RooRealVar mean6("mean6", "mean6",5105.6,5105.6-5*4.53,5105.6+5*4.53);
	RooRealVar sigma6("sigma6", "sigma6", 5.4254,5.4254-5*3.24,5.4254+5*3.24);
	RooGaussian Gauss6("Gauss6", "Gauss6", B_DTF_MM, mean6, sigma6);

	//float sn1 = 1.6683e+03;
	RooRealVar sn1("sn1","sn1",1.6683e+03,1.6683e+03-5*187.,1.6683e+03+5*187.);
	//float sn2 = 1.6567e+03;
	RooRealVar sn2("sn2","sn2",1.6567e+03,1.6567e+03-5*188.,1.6567e+03+5*188.);
	//float sn3 = 1.8110e+01;
	RooRealVar sn3("sn3","sn3",1.8110e+01,1.8110e+01-5*14.2,1.8110e+01+5*14.2);
